...
@Component({
  ...
  imports: [],
  template: `
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700 transition-colors duration-300">
        ...
        <div class="flex gap-2 flex-wrap">
          @for (filterOption of filterOptions; track filterOption.value) {
            <button
              ...
              [class.dark:hover:bg-blue-600]="currentFilter() === filterOption.value"
              [class.hover:bg-gray-200]="currentFilter() !== filterOption.value"
              [class.dark:hover:bg-gray-600]="currentFilter() !== filterOption.value"
            >
              {{ filterOption.label }}
              @if (filterOption.value === 'active' && stats().active > 0) {
                <span class="ml-1 px-2 py-0.5 bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 text-xs rounded-full">
                  {{ stats().active }}
                </span>
              }
              @if (filterOption.value === 'completed' && stats().completed > 0) {
                <span class="ml-1 px-2 py-0.5 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 text-xs rounded-full">
                  {{ stats().completed }}
                </span>
              }
            </button>
